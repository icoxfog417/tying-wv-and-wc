...
def train_baseline(network_size, dataset_kind):
    # prepare the data
    dp = DataProcessor()
    ...
    model.compile()
    model.fit(x_valid, y_valid, x_valid, y_valid, epochs=20)
    model.save(MODEL_ROOT)


def train_augmented(network_size, dataset_kind, tying=False):
    # prepare the data
    dp = DataProcessor()
...
if __name__ == "__main__":
    ...
    if args.aug or args.tying:
        ...
        train_augmented(n_size, dataset, args.tying)
    else:
        train_baseline(n_size, dataset)
